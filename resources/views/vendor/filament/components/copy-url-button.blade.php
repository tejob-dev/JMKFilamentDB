<div>
    <div>
        <div class="flex items-center">
            @if($getRecord()->file)
                <a href="#" onclick="copyUrlToClipboard('{{ asset(\Storage::url($getRecord()->file)) }}')" class="text-indigo-600 hover:text-indigo-900">
            ...
            @endif
            @if($getRecord()->photo)
                <a href="#" onclick="copyUrlToClipboard('{{ asset(\Storage::url($getRecord()->photo)) }}')" class="text-indigo-600 hover:text-indigo-900">
                    <svg fill="#000000" height="24px" width="24px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
                            viewBox="0 0 503.607 503.607" xml:space="preserve">
                        <g transform="translate(1 1)">
                            <g>
            ...
            @if($getRecord()->content)
                <a href="{{ asset('/make/preview/'.$getRecord()?->content_viewable_id.'/'.explode('\\',$getRecord()?->content_viewable_type)[2]) }}" target="_blank" class="text-indigo-600 hover:text-indigo-900">
