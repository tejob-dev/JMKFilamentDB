...
class PublicationsItemRelationManager extends RelationManager
{

    protected static ?string $label = 'Lier une publication et son contenu';

    protected static ?string $title = 'Lier une publication et son contenu';
            ...
            Grid::make(['default' => 0])->schema([
                    ...
                    Forms\Components\Select::make('content_viewable_id')
                        ...
                        ->options(function (callable $get) {
                            ...
                            return Publication::all()->pluck('title', 'id')->toArray();
                        })
                        ->columnSpan([
                            'default' => 12,
                            'md' => 12,
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('content_viewable_id')
                    ->label('Publication')
                    ->getStateUsing(fn ($record) => Publication::find($record->content_viewable_id)?->title)
                    ->sortable()
                    ->searchable(),
            ])
            ->filters([
            ...
            ->headerActions([
                ...
                ->using(function (HasRelationshipTable $livewire, array $data): Model {
                    $dataf = $data;
                    $dataf["content_viewable_type"] = strval(Publication::class);
                    // dd($dataf);
