...
class ActualitesItemRelationManager extends RelationManager
{

    protected static ?string $label = 'Lier une actualité et son contenu';

    protected static ?string $title = 'Lier une actualité et son contenu';
            ...
            Grid::make(['default' => 0])->schema([
                    ...
                    Forms\Components\Select::make('content_viewable_id')
                        ...
                        ->options(function (callable $get) {
                            ...
                            return Actualite::all()->pluck('title', 'id')->toArray();
                        })
                        ->columnSpan([
                            'default' => 12,
                            'md' => 12,
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('content_viewable_id')
                    ->label('Actualité')
                    ->getStateUsing(fn ($record) => Actualite::find($record->content_viewable_id)?->title)
                    ->sortable()
                    ->searchable(),
            ])
            ->filters([
            ...
            ->headerActions([
                ...
                ->using(function (HasRelationshipTable $livewire, array $data): Model {
                    $dataf = $data;
                    $dataf["content_viewable_type"] = strval(Actualite::class);
                    // dd($dataf);
