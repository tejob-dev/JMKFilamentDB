...
class CulturesItemRelationManager extends RelationManager
{

    protected static ?string $label = 'Lier une culture et son contenu';

    protected static ?string $title = 'Lier une culture et son contenu';
            ...
            Grid::make(['default' => 0])->schema([
                    ...
                    Forms\Components\Select::make('content_viewable_id')
                        ...
                        ->options(function (callable $get) {
                            ...
                            return Culture::all()->pluck('title', 'id')->toArray();
                        })
                        ->columnSpan([
                            'default' => 12,
                            'md' => 12,
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('content_viewable_id')
                    ->label('Culture')
                    ->getStateUsing(fn ($record) => Culture::find($record->content_viewable_id)?->title)
                    ->sortable()
                    ->searchable(),
            ])
            ->filters([
            ...
            ->headerActions([
                ...
                ->using(function (HasRelationshipTable $livewire, array $data): Model {
                    $dataf = $data;
                    $dataf["content_viewable_type"] = strval(Culture::class);
                    // dd($dataf);
